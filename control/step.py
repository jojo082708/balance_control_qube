...
def run_step(theta: float, current: float,
             ...
             safety, dt: float,
             rnd: int, timestamp: float,
             alpha: float = 0.0, alpha_dot: float = 0.0,
             mode: str = "impedance") -> tuple:
    """
    執行一個控制週期的完整計算。
    ...
    Args:
        ...
        rnd:       輪次編號
        timestamp: 實驗經過時間 [s]
        alpha, alpha_dot: 倒單擺角度 / 角速度（僅供記錄，阻抗控制不對其補償）
        mode:      記錄用的狀態機模式名稱
    ...
    row = _make_row(rnd, timestamp, mode, theta, theta_d, theta_cmd,
                    omega, alpha, alpha_dot, voltage, current, force_est,
                    K, B, M, Kp, Kd)
    return True, reason, voltage, row


def _make_row(rnd, t, mode, theta, theta_d, theta_cmd, omega,
              alpha, alpha_dot, voltage,
    ...
    return {
        ...
        "alpha_rad":      round(alpha,     6),
        "alpha_dot_rads": round(alpha_dot, 6),
