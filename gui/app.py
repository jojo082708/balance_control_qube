...
class ImpedanceControlPanel(tk.Tk):
    ...
    def _build_left(self, parent):
        self._build_pendulum_state_card(parent)
    ...
    def _build_pendulum_state_card(self, parent):
        """倒單擺狀態機指示燈 + 模式切換按鈕（PRD 3.1 / 5.1）。"""
        cp = self._card(parent, "🎚  倒單擺狀態機")

        row = tk.Frame(cp, bg=self.PANEL); row.pack(fill="x", pady=(0, 6))
        self._mode_leds = {}
        for key, label, on_color in [
            ("swingup",   "起擺中",     self.WARN),
            ("balance",   "已平衡",     self.ACCENT2),
            ("impedance", "阻抗控制中", self.ACCENT),
        ]:
            cell = tk.Frame(row, bg=self.PANEL)
            cell.pack(side="left", expand=True)
            cv = tk.Canvas(cell, width=14, height=14, bg=self.PANEL,
                           highlightthickness=0)
            cv.pack()
            oval = cv.create_oval(1, 1, 13, 13, fill=self.BORDER, outline="")
            tk.Label(cell, text=label, font=("Consolas", 8),
                     bg=self.PANEL, fg=self.SUBTEXT).pack()
            self._mode_leds[key] = (cv, oval, on_color)

        self._switch_btn = self._btn(
            cp, "⇢  切換至阻抗控制", self._request_impedance_switch,
            self.ACCENT, "#000000")
        self._switch_btn.config(state="disabled")
        self._mode_reset_btn = self._btn(
            cp, "↺  重置系統（返回起擺）", self._request_mode_reset,
            self.ORANGE, "#000000")
        self._mode_reset_btn.config(state="disabled")
    ...
    def _do_start(self):
        ...
        self._apply_pendulum_state("swingup", False)
    ...
    def _poll_thread(self):
        if self._ctrl_thread and self._ctrl_thread.is_alive():
            ...
            self._stop_btn.config(state="disabled")
            self._apply_pendulum_state("idle", False)
    ...
    def _request_impedance_switch(self):
        if not self.state:
            return
        self.state.impedance_switch.set()
        self._log("[USER] 請求切換至阻抗控制")

    def _request_mode_reset(self):
        if not self.state:
            return
        self.state.mode_reset.set()
        self._log("[USER] 請求重置系統（返回起擺）")

    def _on_pendulum_state(self, mode_name: str, switch_enabled: bool):
        self.after(0, lambda: self._apply_pendulum_state(mode_name, switch_enabled))

    def _apply_pendulum_state(self, mode_name: str, switch_enabled: bool):
        for key, (cv, oval, on_color) in self._mode_leds.items():
            cv.itemconfig(oval, fill=on_color if key == mode_name else self.BORDER)
        self._switch_btn.config(state="normal" if switch_enabled else "disabled")
        self._mode_reset_btn.config(
            state="normal" if mode_name == "impedance" else "disabled")
