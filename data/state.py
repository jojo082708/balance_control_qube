...
class SharedState:
    def __init__(self):
        ...
        # ── 倒單擺 / 阻抗切換狀態機事件（PRD 3.2, 3.3, 5.1）──────────────────
        # GUI 按下「切換至阻抗控制」時 set；control_loop 消耗後立即 clear。
        self.impedance_switch = threading.Event()
        # GUI 按下「重置系統」時 set；僅在阻抗模式（狀態鎖定）下有意義。
        self.mode_reset       = threading.Event()
    ...
    def reset_for_experiment(self):
        ...
        self.impedance_switch.clear()
        self.mode_reset.clear()
